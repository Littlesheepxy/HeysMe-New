...
export const ScrollStackItem: React.FC<ScrollStackItemProps> = ({
  ...
  <div
    className={`scroll-stack-card relative w-full h-64 my-4 p-8 rounded-[32px] shadow-[0_0_20px_rgba(0,0,0,0.1)] box-border origin-top will-change-transform ${itemClassName}`.trim()}
    style={{
...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const scrollEndedRef = useRef(false);
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    if (scroller) {
      ...
      if (isAtEnd && !scrollEndedRef.current) {
        scrollEndedRef.current = true;
        onScrollEnd?.();
        // 允许页面继续滚动
        scroller.style.overscrollBehavior = "auto";
      } else if (!isAtEnd && scrollEndedRef.current) {
  ...
  return (
    ...
    >
      <div className="scroll-stack-inner -mt-[5vh] px-20 pb-[10rem] min-h-screen">
        {children}
