-- HeysMe 邀请码系统数据库表结构（修复版）
-- 创建时间: 2025-01-27
-- 修复了 PostgreSQL 数组语法错误

-- 启用必要的扩展
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 首先确保 users 表有 metadata 字段
...
-- 邀请码批次表（用于批量管理）
CREATE TABLE IF NOT EXISTS public.invite_code_batches (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
...
ADD COLUMN IF NOT EXISTS batch_id uuid REFERENCES public.invite_code_batches(id) ON DELETE SET NULL;
...
CREATE INDEX IF NOT EXISTS idx_invite_code_usages_invite_code_id ON public.invite_code_usages(invite_code_id);
CREATE INDEX IF NOT EXISTS idx_invite_code_usages_user_id ON public.invite_code_usages(user_id);
CREATE INDEX IF NOT EXISTS idx_invite_code_usages_email ON public.invite_code_usages(email);
CREATE INDEX IF NOT EXISTS idx_invite_code_usages_used_at ON public.invite_code_usages(used_at);

CREATE INDEX IF NOT EXISTS idx_invite_code_batches_created_by ON public.invite_code_batches(created_by);
CREATE INDEX IF NOT EXISTS idx_invite_code_batches_is_active ON public.invite_code_batches(is_active);
...
DROP TRIGGER IF EXISTS update_invite_code_usage_stats_trigger ON public.invite_code_usages;
...
-- 删除现有策略（如果存在）
DROP POLICY IF EXISTS invite_codes_admin_policy ON public.invite_codes;
DROP POLICY IF EXISTS invite_codes_creator_policy ON public.invite_codes;
DROP POLICY IF EXISTS invite_codes_public_read_policy ON public.invite_codes;
DROP POLICY IF EXISTS invite_code_usages_admin_policy ON public.invite_code_usages;
DROP POLICY IF EXISTS invite_code_usages_user_policy ON public.invite_code_usages;
DROP POLICY IF EXISTS invite_code_batches_admin_policy ON public.invite_code_batches;
DROP POLICY IF EXISTS invite_code_batches_creator_policy ON public.invite_code_batches;
