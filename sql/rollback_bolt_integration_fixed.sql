...
-- 执行时间: 2025-01-27
-- 版本: 1.0.1 (修复函数依赖问题)
-- 说明: 撤销所有 Bolt 相关的数据库更改，正确处理依赖关系
-- =====================================
...
-- =====================================
-- 2. 删除 Bolt 专用函数（不影响现有函数）
-- =====================================

-- 删除 Bolt 专用函数（这些函数只被 Bolt 使用）
...
BEGIN
  ...
  -- 检查 Bolt 专用函数是否已删除（保留通用函数）
  SELECT COUNT(*) INTO bolt_function_count
  ...
  IF bolt_function_count > 0 THEN
    RAISE EXCEPTION '❌ 部分 Bolt 专用函数未成功删除，剩余: % 个函数', bolt_function_count;
  END IF;
  ...
  RAISE NOTICE '🗑️ 已删除函数: get_bolt_session_details(), cleanup_bolt_data(), update_bolt_file_updated_at()';
  RAISE NOTICE '🗑️ 已删除所有 Bolt 相关字段和策略';
  RAISE NOTICE '🔒 保留了通用函数: update_updated_at_column() (被其他表使用)';
...
BEGIN
  ...
  RAISE NOTICE '   ✅ 删除了 3 个 Bolt 专用函数';
  RAISE NOTICE '   ✅ 删除了 2 个 Bolt 触发器';
  RAISE NOTICE '   🔒 保留了通用函数 update_updated_at_column()';
  RAISE NOTICE '';
  RAISE NOTICE '⚠️  注意事项:';
  RAISE NOTICE '   - 所有 Bolt 相关数据已被永久删除';
  RAISE NOTICE '   - 通用函数 update_updated_at_column() 被保留（被其他表使用）';
